	...
	<div class="col-lg-7">
		<div class="p-5">
			<div class="text-center"> 
				...
				<form class="user" action="<?php echo base_url('Mahasiswa/update');?>" method="post"> 	<div class="form-group"> 
					...
					<div class="form-group"> 
						...
						<?php 
						$id_user_grup = $baris->grub;
						foreach($admin as $grup){
						$id_grub = $grup->id_grub; 
						$name = $grup->grub;
							if ($id_user_grup == $id_grub) {
								$aksi = 'selected';
								$aksi2 = 'hidden';
						?>
						<option value="<?= $id_grub; ?>" <?= $aksi; ?> <?= $aksi2 ?>><?= $name; ?></option> 
					<?php } ?>
 						<option value="<?= $id_grub; ?>"><?= $name; ?></option> 
						<?php }  ?>
						</select>
					</div> 
